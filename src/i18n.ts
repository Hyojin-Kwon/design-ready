...
(b) No React component library is bundled: render componentRef nodes inline
    from their children with a // from: <ComponentName> comment, and never
    fabricate import paths.
...
(a) foundation/ 토큰(var(--...))은 색상·타이포·radius·stroke·gap/padding에만 사용.
    ...
    (calc(var(--a) + var(--b)) 금지).
(b) React 컴포넌트 라이브러리는 포함돼 있지 않음: componentRef 노드는 children으로
    인라인 렌더하고 // from: <ComponentName> 주석을 남길 것. import 경로를 지어내지 말 것.
