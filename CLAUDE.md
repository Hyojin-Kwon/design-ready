...
**평가 항목 (가중치 기반 스코어링):**
- 시맨틱 네이밍 비율 (30%) — 디폴트 이름 vs 의미 있는 이름
- 오토레이아웃 적용률 (25%) — flexbox 변환 가능 여부
- 디자인 시스템 컴포넌트 인스턴스 유지율 (20%) — 디태치 없이 바인딩 유지
- 스타일 토큰 연결률 (15%) — 하드코딩 컬러/타이포 vs 토큰 참조
- 구조 깊이 적정성 (10%) — 불필요한 중첩 없는 깔끔한 트리
