...
const Subtab: React.FC = () => (
  ...
  >
    <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 59 }}>
      {/* Icon 1 (24×24) — back arrow */}
      <Icon name="subtabBack" style={{ width: 24, height: 24 }} />
      {/* Title — TEXT chars verbatim */}
      <span
        style={{
          width: 177,
          height: 26,
    ...
    {/* Frame 17 — right text button (text.chars is empty, render empty span) */}
    <span
      style={{
        width: 75,
        height: 19,
...
const MenuRow: React.FC<RowProps> = ({ label, iconName, iconLeft, iconTop, iconW, iconH, fontSize = 13, labelWidth }) => (
  <div style={{ position: 'relative', width: 375, height: 50, backgroundColor: 'var(--neutral-colors-linewhite)' }}>
    ...
    <span
      style={{
        ...
        top: 16,
        width: labelWidth,
        height: fontSize === 15 ? 18 : 20,
        fontFamily: fontIos,
        fontSize:
          fontSize === 15
            ? 'var(--font-size-ios-font-size-text-15)'
            : 'var(--font-size-ios-font-size-text-13)',
        fontWeight: w4,
        color: 'var(--neutral-colors-linegray900)',
        lineHeight: fontSize === 15 ? 1.2 : 1.5,
      }}
    >
      {label}
    </span>
...
const ProfileRow: React.FC = () => (
  <div style={{ position: 'relative', width: 375, height: 50, backgroundColor: 'var(--neutral-colors-linewhite)' }}>
    ...
    {/* Chevron */}
    <Icon name="chevronMore" style={{ position: 'absolute', left: 351, top: 20, width: 6, height: 11 }} />
    {/* Right Row: 17×17 photo ellipse + "同期会" label */}
    <div
      style={{
        position: 'absolute',
        left: 275,
  ...
  <div style={{ position: 'relative', width: 375, height: 136, backgroundColor: 'var(--neutral-colors-linewhite)' }}>
    ...
    <div
      style={{
        ...
        width: 85,
        height: 85,
        backgroundColor: '#F5F5F5', // TODO: token? #F5F5F5 not in token list
      }}
    >
      <DimmedOverlay />
...
const AlbumSection: React.FC = () => (
  <div style={{ position: 'relative', width: 375, height: 136 }}>
    ...
    <AlbumCard left={16} title="Home Party" titleWidth={77} count="72" countLeft={92} countWidth={16} />
...
const AlbumCard: React.FC<{ left: number; title: string; titleWidth: number; count: string; countLeft: number; countWidth: number }> = ({
  left,
  title,
  titleWidth,
  count,
...
const DescriptionRow: React.FC = () => (
  ...
  >
    <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 5, height: 17 }}>
      ...
      {/* Group 2085673306 — help text + arrow */}
      <div style={{ position: 'relative', width: 236, height: 17 }}>
        ...
        <Icon name="arrowRightSmall" style={{ position: 'absolute', left: 274, top: 3, width: 7, height: 11 }} />
      </div>
    </div>
  </div>
);
...
const ChatMenu: React.FC = () => (
  ...
  >
    ...
    {/* Stack: 375×622 at y=184, vertical flex (gap=0, paddingBottom=21) */}
    <div
      style={{
        position: 'absolute',
        left: 0,
    ...
    >
      ...
      {/* Stack[9] 設定 — absolute at (0,601) inside Stack (absolute-in-autolayout) */}
      <div style={{ position: 'absolute', left: 0, top: 601 }}>
        <MenuRow label="設定" labelWidth={30} iconName="settings" iconLeft={16} iconTop={15} iconW={20} iconH={20} fontSize={15} />
      </div>
    </div>
